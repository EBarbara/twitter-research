...
class data_creation_word2vec():
    ...
    def __init__(self, filename_train, filename_test, word2vec_type, classdataset):
        ...
        # Parse train data
        filename = 'Data Collection/' + filename_train
        ...
        # Parse test data
        filename = 'Data Collection/' + filename_test
        with open(filename, 'r', encoding='utf-8', newline='') as f:
            reader = csv.reader(f)
            test_tweets = []
            for tweet in reader:
        ...
        if word2vec_type == 'Twitter':
            ...
            self.Word2Vec_model = KeyedVectors.load_word2vec_format('Models/word2vec_twitter_model.bin', binary=True,
                                                                    encoding='latin-1')
        if word2vec_type == 'Google' or word2vec_type == 'random':
            ...
            self.Word2Vec_model = KeyedVectors.load_word2vec_format('Models/GoogleNews-vectors-negative300.bin', binary=True)
    ...
    def data_creation(self):
        if self.word2vec_type == "Twitter" or self.word2vec_type == 'Google':
            ...
            Test_Y_ori = [int(tweet[0]) for tweet in self.test_tweets]
            Train_Y = [int(tweet[0]) for tweet in self.training_tweets]
            filename = 'Models/1_Word2Vec_' + self.word2vec_type + '_' + self.classdataset + '.pickle'
            with open(filename, 'wb') as f:
                pickle.dump([Train_X, Test_X, Train_Y, Test_Y_ori], f)
...
if __name__ == '__main__':
    word2vec = data_creation_word2vec(filename_train='1_TrainingSet_3Class.csv', filename_test='1_TestSet_3Class.csv',
                                      word2vec_type='Twitter', classdataset='3class')
